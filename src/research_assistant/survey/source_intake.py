...
def build_source_intake_metadata_authority(
    *,
    ...
    if mission_input_view(snapshot.contract)["input_mode"] == TOPIC_INPUT_MODE:
        raise MissionStateError(
            "topic_bootstrap_metadata_authority_required",
            "topic missions require a separately validated bootstrap-bound metadata artifact before source intake",
        )
...
def run_mission_source_intake(
    *,
    ...
    if mission_input_view(snapshot.contract)["input_mode"] == TOPIC_INPUT_MODE:
        raise MissionStateError(
            "topic_bootstrap_metadata_authority_required",
            "topic missions require bootstrap-bound metadata authority before source intake",
        )
    mission = mission_root.absolute()
...
def validate_mission_source_intake(
    *,
    ...
    if mission_input_view(snapshot.contract)["input_mode"] == TOPIC_INPUT_MODE:
        raise MissionStateError(
            "topic_bootstrap_metadata_authority_required",
            "M17 does not define a topic-mode source-intake artifact family",
        )
    mission = mission_root.absolute()
    intake_root = mission / "source_intake"
    records_root = mission / "local_research" / "papers" / "source" / "records"
