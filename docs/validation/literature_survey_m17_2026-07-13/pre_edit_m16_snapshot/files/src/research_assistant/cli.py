...
def build_parser() -> argparse.ArgumentParser:
    ...
    survey_run = survey_sub.add_parser(
        ...
        description='Supervise the public-source survey workflow from topic+seed. By default this writes mission_control.json and next_action.json and asks one public-discovery question. --confirm-public-discovery records that confirmation and may run bounded public metadata discovery; it does not allow credentials, private databases, paid model workers, hidden evaluator material, unbounded crawling, claim support from metadata, or final prose readiness.',
    )
    survey_run.add_argument('--topic', required=True)
    survey_run.add_argument('--seed', action='append', required=True, help='Seed paper identifier; repeat for multiple seeds')
